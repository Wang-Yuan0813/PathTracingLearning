...
public:
	...
	Ray generateRay(float x, float y)
	{
		...
		float ndcY = (1.0 - y / height) * 2.0f - 1.0f;

		Vec3 rayDir(ndcX, ndcY, 1.0f);
		rayDir = inverseProjectionMatrix.mulPointAndPerspectiveDivide(rayDir);
		rayDir = camera.mulVec(rayDir);
...
public:
...
#if BVH
	IntersectionData traverse(const Ray& ray) {//BVH traversal
		return bvh->traverse(ray, triangles);
	}
#else
	IntersectionData traverse(const Ray& ray)//naive traversal
	{
	...
	Light* sampleLight(Sampler* sampler, float& pmf)
	{
		...
		if (lights.empty()) return NULL;
		pmf = 1 / (float)lights.size();
		Light* light = lights[(int)(sampler->next() * lights.size())];
		return light;
		//return NULL
	}
	...
	{
		IntersectionData intersection = traverse(ray);
		if (intersection.t == FLT_MAX){
			return false;
		}
		return true;
